...
#		$fullName was seen in the results.
#	$UnableToFindMIInRSIDN{"$fullName":OrgCourse} = a list of 1 or more "$org:$course" strings which
#		denote the result file we can find this swimmer in.
my %UnableToFindMIInRSIDN = ();
...
### FetchStats
    				...
    				#			alter table FetchStats add FS_ePostalPointEarners INT DEFAULT 0;
				  	...
				  	# --- FS_OWLines - number of OW lines generated by PMSOWPoints for use by AGSOTY
				  	# 
		    		($sth,$rv) = PMS_MySqlSupport::PrepareAndExecute( $dbh, 
...
#	3) As of 2022 we have found names in RSIND that don't have a middle initial but the SAME
...
sub GetDetailsFromFullName( $$$$$$$$ ) {
	...
	# (Unless the RSIDN entry for this swimmer is missing a middle initial and the passed name includes
	# a middle initial.  Problem discovered Nov, 2022. See below.)
	...
	if( scalar @regNum > 0 ) {
		...
		###**************************************************************************************
		# UPDATE Nov, 2022: USMS is re-registering swimmers who previously registered with a
		#	middle initial with a name without the middle initial. This means that sometimes
		#	we'll see a result with the name "xxx Y zzzz" but RSIND has their name as "xxx zzzz".
		#	So, for that reason, if the passed $fullName resulted in our @arrOfBrokenNames containing
		#	names with a middle initial then we'll try those names WITHOUT that middle initial.
		#	For example, if the passed $fullName was "xxx Y zzzz", which resulted in a broken name
		#	in our @arrOfBrokenNames with the middle initial "Y", we'll then try that broken name
		#	with the "Y" middle initial removed to see if we find "xxx zzzz" in our RSIND file.
		

		# try valid names with the middle initial removed:
		my @arrOfNamesNoMI = RemoveMiddleInitial( \@arrOfBrokenNames );
		if( scalar( @arrOfNamesNoMI ) > 0 ) {
			# now march through this new set of name possibilities and see if we can find this swimmer in the RSIDN
			for( my $nameIndex = 0; $nameIndex < scalar @arrOfNamesNoMI; $nameIndex++ ) {
				my $hashRef = $arrOfNamesNoMI[$nameIndex];
				# see if this set of first/last names matches a name in our RSIDN table (require RSIND name to have an empty middle initial)
				TT_MySqlSupport::GetRegnumFromName( $fileName, $lineNum, 
					$hashRef->{'first'}, "", $hashRef->{'last'}, 0,
					\@regNum, \@teamInitials, \@DOB, \@firstName, \@middleInitial, \@lastName );
			}
		}
		# we're done trying all the different possibilities for this swimmer's name with the middle initial being empty.  
		# Same logic as above:  If we have exactly one 
		# match in the RSIDN table then we're good.  If we have 0 then this swimmer isn't a PAC swimmer.
		# If we have more than one match then the full name is ambiguous and matches more than one PAC
		# swimmer, thus we don't know who to award points to.  In that case we're going to try another
		# heuristic.
		if( scalar @regNum > 0 ) {
			...
			PMSLogging::DumpWarning( "", "", "TT_MySqlSupport::GetDetailsFromFullName(): Couldn't find regnum for swimmer '$fullName'.  " .
				...
				"\n    BUT when we ignore their middle initial we found some RSIND entries: " .
				"\n    RegNums: " . join( ", ", @regNum) . 
				", FirstNames: " . join( ", ", @firstName ) . 
				", LastNames: " . join( ", ", @lastName ) .
				"\n    The name we've decided to use is this:" .
				"\n    RegNum: $returnedRegNum, FirstName: $returnedFirstName, MiddleInitial: $returnedMiddleInitial, " .
				"LastName: $returnedLastName", 1 );

			# remember this issue to be logged later is a summary of like issues:
			my $count = $UnableToFindMIInRSIDN{$fullName};
			$count = 0 if( !defined $count );
			$UnableToFindMIInRSIDN{$fullName} = $count+1;
			# remember the org and course we're seeing this problem in.  
			# $UnableToFindMIInRSIDN{"$fullName:OrgCourse"} is of the form:
			#    org:course[,org:course:...]
			if( !defined $UnableToFindMIInRSIDN{"$fullName:OrgCourse"} ) {
				$UnableToFindMIInRSIDN{"$fullName:OrgCourse"} = "$ageGroup;$org:$course";
			} elsif( $UnableToFindMIInRSIDN{"$fullName:OrgCourse"} !~ m/$org:$course/ ) {
				$UnableToFindMIInRSIDN{"$fullName:OrgCourse"} .= ",$org:$course";
			}
			...
			# scalar @regNum == 0
			###**************************************************************************************
			# We're completely out of luck - we can't find this swimmer in our RSIND file.
			if( ! $errorFlag ) {
		...
		} # end of scalar @regNum == 0 using Nov 2022 Update
	} # end of scalar @regNum == 0 
...
sub GetRegInfoForSwimmer( $$$$$$$$$$$$$ ) {
	...
	my @DOB = @$DOBRef;
	my @firstName = @$firstNameRef;
	my @middleInitial = @$middleInitialRef;
	my @lastName = @$lastNameRef;
	my ($returnedFirstName, $returnedMiddleInitial, $returnedLastName) = ("","","");
	my ($returnedRegNum, $returnedTeamInitials) = ("", "");
	my $yearBeingProcessed = PMSStruct::GetMacrosRef()->{"YearBeingProcessed"};
...
# 		my @arrOfNamesNoMI = RemoveMiddleInitial( \@arrOfBrokenNames );
# RemoveMiddleInitial - Generate a list of "first" and "last" names from the passed list of
#	"first", "middle", and "last" names.
#
# PASSED:
#	$arrOfBrokenNamesRef - a reference to an array of "broken" names, where a broken name
#		is represented by a hash with keys "first", "middle", and "last".  This is basically the
#		same as the array generated by BreakFullNameIntoBrokenNames()
#
# RETURNED:
#	@arrOfNamesNoMI: an array of hashes:
...
# NOTES:
#	For every name in the passed @arrOfBrokenNames in the form:
#		arr[n]->{'first'} is a possible first name
#		arr[n]->{'middle'} is non-empty
#		arr[n]->{'last'} is the matching possible last name
#	this routine will return the passed name in @arrOfNamesNoMI:
#		arr[k]->{'first'} is the same as arr[n]->{'first'}
#		arr[k]->{'middle'} is ""
#		arr[k]->{'last'} is the same as arr[n]->{'last'}
#	Every passed name where the middle initial is empty will be ignored.
#
#	Nov, 2022: USMS is re-registering swimmers who previously registered with a
#	middle initial with a name without the middle initial. This means that sometimes
#	we'll see a result with the name "xxx Y zzzz" but RSIND has their name as "xxx zzzz".
#	So, for that reason, if the passed $fullName resulted in our @arrOfBrokenNames containing
#	names with a middle initial then we'll try those names WITHOUT that middle initial.
...
sub RemoveMiddleInitial( $ ) {
	...
	# handle the Nov, 2022 issue described above:
	if( scalar( @arrOfBrokenNames ) > 0 ) {
		foreach my $singleFullName( @arrOfBrokenNames ) {
			if( $singleFullName->{'middle'} ne "" ) {
				my $namesRef = {};
				$namesRef->{'first'} =  $singleFullName->{'first'};
				$namesRef->{'middle'} =  "";
				$namesRef->{'last'} =  $singleFullName->{'last'};
				push( @result, $namesRef );
			}
		}
	}
	return @result;
} # end of RemoveMiddleInitial()
...
sub DumpErrorsWithSwimmerNames() {
	...
	if( my $size = scalar keys %UnableToFindMIInRSIDN ) {
		$size /= 2;		# divide size by two since a hash has 2 elememts per swimmer (key and value)
		# dump out the names of all swimmers that we think we should have found in the RSIDN that we didn't find
		...
		foreach my $key (keys %UnableToFindMIInRSIDN) {
			...
			PMSLogging::PrintLog( "", "", "    '$key' (appeared in " . $UnableToFindMIInRSIDN{$key} . 
				" results [" . $UnableToFindMIInRSIDN{"$key:OrgCourse"} . "])" );
		}
	}
...
#
# PASSED:
#	$season - the year being processed
#	$raceLines - (not used...)
#	$PMSSwimmerData - full path name of directory holding RSIND file
#
# RETURNED:
#	n/a
#
...
#	contains the string "Results have changed" (case sensitive)then programs looking at this log will know
	...
	if( $numRows == 1 ) {
		...
		if( my $numDiffs = TT_Struct::HashesAreDifferent( TT_Struct::GetFetchStatRef(), $resultHash ) ) {
			...
			if( 
				...
				PMSLogging::PrintLog( "", "", "It appears that Results have changed ($numDiffs changes) " .
		...
		if( defined $swimmerDataFile ) {
			...
			if( $refreshRSIDNFile == 1 ) {
				...
				"Results have changed. The new RSIND file is:\n   $simpleName ", 1 );
				$resultsHaveChanged = 1;
			} else {
