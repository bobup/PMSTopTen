...
*/
function OpenSwimmer( passedID, genAgeGroupFileName ) {
	var result;
	...
	if( (passedID != "") && document.getElementById( passedID ) !== null ) {
		...
		var remoteFile = "HTMLVSupport/" + genAgeGroupFileName + ".html";
		$("#"+virtSectionId).load( remoteFile, function() {
			...
			$("#pageStats").html(OriginalPageSize + " / " + PreviousPageSize + " / " +  CurrentPageSize + " (" + diff + ")" );
...
// Used to show the SsOTY (Swimmers Of The Year)
// Look for the query:  "?soty=x" for any x
$(document).ready(function() {
	var query = getQueryVariable("soty");
	if( query ) {
		var remoteFile = "HTMLVSupport/soty.html";
		$("#soty").load( remoteFile, function() {
			// this function is disabled... - see ShowSoty()
		}  // end of anonymous function
		);  // end of load()
	} 
});
...
$(document).ready(function() {
	...
	$("#pageStats").html(OriginalPageSize);
});
...
function ShowSoty(event) {
	// just show or hide some existing DOM:
	var mydisplay = document.all['details'].style.display;
	// recognize this click ONLY if both the shift key and the option (Mac) or alt (Win) key are pressed during the click
	if( (mydisplay == "none") && (event.shiftKey) && (event.altKey) ) {
